//var graserv="http://ric304017.ricerca.lan/cgi-bin/owtchart.exe";
//var bootstrapserv="http://ric304017.ricerca.lan/bt/prod";
...
function setURLParam() {
	...
	ModCalc.VelMed = {
		"25": getURLParameter("Velmed25"),
		"50": getURLParameter("Velmed50"),
		"75": getURLParameter("Velmed75"),
		"100": getURLParameter("Velmed100"),
	};
	ModCalc.Produc = {
		"25": getURLParameter("Produc25"),
		"50": getURLParameter("Produc50"),
		"75": getURLParameter("Produc75"),
		"100": getURLParameter("Produc100"),
	};
	ModCalc.DistanzaImp = getURLParameter("Distimp");
	ModCalc.KWeib = getURLParameter("KWeib");
	...
	VelMed[0]=ModCalc.VelMed["25"];
	VelMed[1]=ModCalc.VelMed["50"];
	VelMed[2]=ModCalc.VelMed["75"];
	VelMed[3]=ModCalc.VelMed["100"];
	Produc[0]=ModCalc.Produc["25"];
	Produc[1]=ModCalc.Produc["50"];
	Produc[2]=ModCalc.Produc["75"];
	Produc[3]=ModCalc.Produc["100"];
	VelMedAnn = Interpola(25,VelMed.slice(),Produc.slice());
	KWeib = getURLParameter("KWeib");
...
//fine combo ---------------------------------------------------------------------------
	...
	//	console.log(t);
		...
		$("[id^=hgene]").click(function(){
			str=this.id;
			idh=str.substring(5); 
			var str = __('Height: ') + r[5 + Number(idh)] + " m";
			...
			document.getElementById("VelMedAnnua").value = Vinterp.toFixed(2);  //Interpola(r[5 + Number(idh)],VelMed);
		...
		// urlchart= urlchart + 'cgi-bin/owtchart.exe?Type=Combo_Line_Line&W=800&H=300' + 
					// '&NumSets=1&NumPts='+ totore + '&YMin=' + YMin + '&YMax=' + YMax + 
					// '&YMin2=' + YMin2 + '&YMax2=' + YMax2 + '&Yaxis2=1' + 
					// '&Vals=' + strout + '&Title=' + titolo + 
					// '&YTitle=' + YTitle +'&YTitle2=' +YTitle2 + '&XLabels=' + XLabels +
					// '&YLabel2Fmt=' + YLabel2Fmt + '&YTitle2Color=000000&PlotColor=FF0000&VolColor=0000FF';
	...
	//estrae la curva dal Dbase e la plotta
		...
		$.getJSON(url,function(result){
			...
			// for (i = 0; i < lu; i++) {
				// console.log("Vnom" + Velnom[i] + "  Pnom" + Potnom[i] + "  Vcp" + Velcp[i] + "  Pcp" + Potcp[i] + "  V" + VelAgen[i] + "  P" + PotAgen[i] )
			// }			
...
// function cum_weib(x,alfa,beta) {
// //cumulata distrib weibull
// //alfa=fatt forma (k);  beta=vel caratteristica (A)
// var out = 1 - Math.exp(-Math.pow((x/beta),alfa));
// return out;
// };

// function dens_weib(x,alfa,beta) {
// //densità di probabilità distrib weibull
// //alfa=fatt forma (k);  beta=vel caratteristica (A)
// var out = (alfa / Math.pow(beta,alfa)) * Math.pow(x,(alfa - 1)) * Math.exp(-Math.pow((x/beta),alfa));
// return out;
// };

// function Interpolaold(altmoz,VelMed,Produc) {
	// var altezze = [25,50,75,100];
	// var sett, Vint, Pint, y1, y2, v1, v2, p1, p2;
	// sett = 0;
	...
	// y2 = altezze[sett+1];
	// y1 = altezze[sett];
	// v2 = Number(VelMed[sett+1]);
	// v1 = Number(VelMed[sett]);
	// p2 = Number(Produc[sett+1]);
	// p1 = Number(Produc[sett]);
	
	// Vint = (v2 - v1) / (y2 - y1) * (altmoz - y1) + v1
	// Pint = (p2 - p1) / (y2 - y1) * (altmoz - y1) + p1
	// console.log(altmoz + " " + sett + " y:" + y1 + " " + y2 + " v:" + v1 + " " + Vint + " " + v2 + " p:" + p1 + " " + Pint + " " + p2 );
// };
